...
from kikuchipy.detectors import ECPDetector
...
class ECP(CommonImage, Signal2D):
    ...
    _signal_type = "ECP"
    _alias_signal_types = ["electron_channelling_pattern"]
    _lazy = False
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.metadata.has_item(metadata_nodes("ecp")):
    ...
    @property
    def detector(self) -> ECPDetector:
        """An :class:`~kikuchipy.detectors.ecp_detector.ECPDetector`
        describing the ECP detector dimensions, the projection/pattern
        centre, and the detector-sample geometry.
        """
        return self._detector

    @detector.setter
    def detector(self, value: ECPDetector):
        ...
        **kwargs,
        ...
        if not self._lazy:
            ...
            with ProgressBar():
                ...
                background_signal = ECP(background_return)
        else:
            background_signal = LazyECP(background_patterns)
...
class LazyECP(LazySignal2D, ECP):
